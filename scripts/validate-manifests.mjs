...
assert.equal(workflows.count, 360);
assert.equal(workflows.items.length, 360);
assert.equal(new Set(workflows.items.map((item) => item.id)).size, 360);
assert.ok(workflows.source.repo && workflows.source.ref);
assert.equal(agents.count, 60);
assert.equal(agents.items.length, 60);
assert.equal(new Set(agents.items.map((item) => item.id)).size, 60);
...
console.log('Manifest validation passed: 360 workflows, 60 agents, 500 planned assets.');
